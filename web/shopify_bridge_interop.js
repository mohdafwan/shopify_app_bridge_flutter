...
(function () {
  ...
  window.ShopifyBridgeHelper = {
    ...
    /** Sets the App Bridge TitleBar title. */
    ...
    /** Starts the App Bridge full-page Loading indicator. */
    showLoading: function (app) {
      if (typeof shopify !== 'undefined') {
        if (typeof shopify.loading === 'function') {
          shopify.loading(true);
        } else {
          // fallback if shopify.loading is an object
    ...
    /** Stops the App Bridge full-page Loading indicator. */
    ...
    /** Shows an App Bridge Modal dialog. */
    showModal: function (app, title, message) {
      if (typeof shopify !== 'undefined') {
        var modalId = 'shopify-flutter-modal';
        var m = document.getElementById(modalId);
        if (m) m.remove(); // recreate to ensure fresh content
      ...
      if (ab && ab.actions && ab.actions.Modal && realApp) {
        ...
        var modal = Modal.create(realApp, {
          title: title,
          message: message
        });
